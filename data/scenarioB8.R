...
periods_scenB8 <- read_grazing_periods(x)
...
grain_ingest_par_fememlle_scenB8  <- output_consum[[1]]
forage_ingest_par_fememlle_scenB8  <- output_consum[[2]]
light_young_scenB8  <- output_consum[[3]]
heavy_young_scenB8  <- output_consum[[4]]
female_culled_scenB8  <- output_consum[[5]]
...
plot_man_scenB8 <- as.data.frame(by(a$taille, list(a$id_mod_expl_comp), sum))
plot_man_scenB8 <- cbind(rownames(plot_man_scenB8), plot_man_scenB8)
colnames(plot_man_scenB8 ) <- c("Surface", "taille")
